...
// Acesso direto às posições de memória
// Clock
...
// Mudança de estados do LED

// Função que atualiza os estados do LED (cores)
...
void EXTI15_10_IRQHandler(void) {
    if (EXTI_PR1 & (1 << 13)) { // Caso o botão seja apertado (comando do PC13)

        // Limpa flag de interrupção
        EXTI_PR1 |= (1 << 13); // Escreve "1" bit 13
...
int main(void) {
    ...
    // Habilita clocks (enable)
    RCC_AHB4ENR |= (1 << 3); // GPIOD (LED)
    RCC_AHB4ENR |= (1 << 2); // GPIOC (Botão)
